...
This is a guide to set up n8n MCP ( both [community MCP](https://github.com/czlonkowski/n8n-mcp/) and official MCP ) with Codex on Windows.
...
- The default in this guide is `n8n-mcp@latest` because it is the simplest setup for most people.
- `n8n-mcp@latest` checks for the newest community MCP when Codex starts.
- A pinned version like `n8n-mcp@1.2.3` is the stable option and only changes when you edit the version number.
- You do not need to switch back and forth. Pick the style you want and keep it.
...
- NEVER touch live workflows until:
  ...
  2. Inputs and credentials are clear.
...
### Option 2 ( My Personal Workflow Preference )
...
- Never store API tokens, secrets, or access tokens in:
  * `Set` nodes.
  * Webhook payloads.
  * Expressions.
  * Hardcoded headers.
...
- Always use n8n credentials directly on nodes for authentication.

- Only keep non-secret values in workflow placeholders, such as:
  * Internal base URLs.
  * Phone number IDs.
  * Sheet IDs.
  * Email addresses.
...
- If a constant base URL is used only once in a workflow, inline it directly in the node.

- If a non-secret constant is reused by multiple downstream nodes, it may be stored once in a `Set` node for clarity.
```
...
## 10. Public Webhooks / Tunneling / Hosting

If you need Stripe, Telegram, GitHub, Typeform, Meta, or any other external service to call your local n8n, follow the separate guide here:

- [2. Tunneling Guide](./2.%20tunneling%20guide.md)

The guide to hosting on VPS is also in this guide.
