...
This is a guide to setup n8n mcp with Codex on Windows.

This gets you:

- local `n8n` in Docker
- `n8n_docs` in Codex for node search and validation
- `n8n_live` in Codex for your real local instance
...
Create the Docker volume and start n8n:
...
Save the token at user scope so the Codex desktop app can read it after restart:
